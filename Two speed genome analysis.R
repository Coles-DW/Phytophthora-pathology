...
BiocManager::install("fields", force = TRUE)
BiocManager::install("EBImage")
BiocManager::install("TSSi")

chooseCRANmirror()
install.packages(c("fields", "TSSi"))
...
x <- import.gff("Mygtf2.gtf")#does dont differ from mygtf.gtf (same file no change i.e original obtained from Github)
x2<- import.gtf("Mygtf2.gtf")
gff<-import.gff("Mygff2.gff")
gff2<- import.gff("Mygff.gff")

## Set "group_col" to gff metadata field with unique gene identifiers.
gffgene<-getFeat2b(x=gff,format="GFF", group_col="ID", range_types=c("gene"))

strand(gffgene) <- mcols(gffgene)$score

mcols(gffgene)$score<-NULL

gffintg<-getFeat2b(x=gff, format="GFF",group_col="ID", range_types=c("intergenic"))
...
write.table(FIRdata,file="PM_MyFIRs.csv", sep=",", row.names=FALSE)
...
write.table(BinLimits,file="PM_MyBins.txt")
...
BinLimits<-as.numeric(unlist(read.table(file="PM_MyBins.txt", header=TRUE, row.names=1)))

## Alt. import own FIR data
FIRdata<-read.csv(file="PM_MyFIRs.csv",sep=",")
xbin=cut(FIRdata$fiveprime, breaks= c(BinLimits))
ybin=cut(FIRdata$threeprime, breaks= c(BinLimits))
FIRdata<-cbind(FIRdata, xbin, ybin, 
               genevalue=rep(1, length(FIRdata$fiveprime)))
...
filled.contour(x, y, z=GenValMatrix, 
               plot.title = title(main ="Phytophthora medicaginis genome", 
...
rxlrData<-as.data.frame(read.csv('PM_RxLR_FIRs_2.csv', header=TRUE))
rxlrData<-as.data.frame(read.csv('PM_RxLR_FIRs_3.csv', header=TRUE))
...
ggplot(data=rxlrData, aes(x=rxlrData$rxlr_five, y=rxlrData$rxlr_three, geom="blank")) + 
  annotation_custom(g, xmin=-Inf, xmax=Inf, ymin=-Inf, ymax=Inf) +
  coord_fixed(ratio=1) +
  geom_point(shape=21, fill="white", colour="black", size=4, alpha=0.7, na.rm=FALSE, position = position_jitter(width=0.01, height=0.01)) +
  scale_fill_manual(values = c('rxlr'="red")) +
  scale_y_log10(breaks=BinLimits[2:length(BinLimits)], limits=c(BinLimits[2], BinLimits[NumBins +1])) +
  scale_x_log10(breaks=BinLimits[2:length(BinLimits)], limits=c(BinLimits[2], BinLimits[NumBins +1])) +
  ggtitle("RXLR Gene Intergenic Flanking Distance \n vs Global Intergenic Distance ") +
